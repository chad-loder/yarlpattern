...
WPT_DIR = REPO / "reference" / "wpt" / "urlpattern" / "resources"
...
def _render(suites: list[SuiteResult]) -> str:
    ...
    out.append(
        ...
        f"running against the canonical "
        f"[`web-platform-tests/wpt/urlpattern/`](https://github.com/web-platform-tests/wpt/tree/master/urlpattern) "
        f"corpus with regex engine **`{engine.name}`** "
    ...
    for suite in suites:
        ...
        out.append("")
        out.append(
            f"Source: [`{suite.source}`](https://github.com/web-platform-tests/wpt/blob/master/urlpattern/resources/{suite.source})"
        )
        out.append("")
...
def main() -> int:
    ...
    suite = SuiteResult(title="Data corpus", source="urlpatterntestdata.json")
    ...
    # 2. Constructor (code-driven; harness inline)
    suite = SuiteResult(title="Constructor edge cases", source="urlpattern-constructor.any.js")
    suite.cases = _run_constructor_inline()
    suites.append(suite)

    # 3. hasRegExpGroups (code-driven; harness inline)
    suite = SuiteResult(title="hasRegExpGroups semantics", source="urlpattern-hasregexpgroups-tests.js")
    suite.cases = _run_hasregexpgroups_inline()
    suites.append(suite)

    # 4. compareComponent
    compare_path = WPT_DIR / "urlpattern-compare-test-data.json"
    suite = SuiteResult(title="compareComponent (tentative spec)", source="urlpattern-compare-test-data.json")
    ...
    suite = SuiteResult(title="generate (tentative spec)", source="urlpattern-generate-test-data.json")
